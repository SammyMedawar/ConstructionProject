...
{
    public partial class ucSalesPayment : UserControl
    {
        ...
        //create singleton
        private static ucSalesPayment _instance;
        public static ucSalesPayment Instance
        {
            get
            {
                if (_instance == null)
                    _instance = new ucSalesPayment();
                return _instance;
            }
        }
        ...
        public void loadData()
        {
            ...
            try
            {
                ...
                string query = "SELECT ID, CompanyName, PayDate, ToPay, Paid, CumulativeTotal FROM SaleHistory WHERE CompanyName like '" + comboCompany.Text + "' AND PayDate BETWEEN '" + dtPickerFrom.Value.ToString("yyyy-MM-dd HH:mm:ss") + "' AND '" + newTo.ToString("yyyy-MM-dd HH:mm:ss") + "' ORDER BY PayDate ASC, ToPay DESC, CumulativeTotal DESC  LIMIT " + limit +";";
        ...
        private String generateID()
        {
            ...
            String lastID = "";
            String query = "SELECT ID FROM SinglePaymentSales ORDER BY DateNow DESC Limit 1";
            SQLiteCommand cmd = new SQLiteCommand(query, con);
            SQLiteDataReader dr = cmd.ExecuteReader();
            if (dr.Read())
            {
            ...
            String newestID;
            newestID = "DSP" + currentYear + "-" + newestIDNumber;
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string query = "SELECT CumulativeTotal FROM SaleHistory WHERE CompanyName LIKE '" + comboCompany.Text + "' ORDER BY PayDate DESC LIMIT 1";
                SQLiteCommand cmd = new SQLiteCommand(query, con);
                SQLiteDataReader dr = cmd.ExecuteReader();
                if (dr.Read())
                {
            ...
            {
                MessageBox.Show("An Error has occured! Please restart the applicaction");
                con.Close();
                return;
            }
            ...
            using (var trans = con.BeginTransaction())
            {
                try
                {
                    ...
                    //Insert into Sale history
                    string query2 = "INSERT INTO SaleHistory VALUES (@ID, @PayDate, 0, @Paid, @cum - @Paid, @CompanyName )";
                    ...
                    //Update company info
                    string query3 = "UPDATE Company SET CumulativeOwedToUs = @cum WHERE CompanyName LIKE @CompanyName";
                    ...
                    string query4 = "INSERT INTO SinglePaymentSales VALUES (@ID, @Now, @CompanyName)";
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please make sure there are payments to print!");
            }
        }

        private void ucSalesPayment_Load(object sender, EventArgs e)
        {
